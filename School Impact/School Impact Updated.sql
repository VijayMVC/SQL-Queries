SELECT
    ...
    round(sum(case when attd.school_key = all_enr.previous_school_key then attd.attendance_value else null end) / sum(case when attd.school_key = all_enr.previous_school_key then attd.attendance_days else null end),3) as prev_attd_rate,
...
FROM
    ...
    INNER JOIN
        ...
        WHERE
                (wd_cd.date_value - adm_cd.date_value > 44 or (wd_cd.date_value > sysdate and sysdate - adm_cd.date_value > 44))
               -- and st.student_key = 146695
                and schaa.reporting_school_ind = 'Y'
--                                             and st.student_id = '8704651'
        GROUP BY
                st.student_key,
                st.student_id,
                schaa.school_key,
    ...
    FROM
        ...
        INNER JOIN K12INTEL_DW.DTBL_SCHOOL_DATES sd on d.school_dates_key = sd.school_dates_key  and sd.local_school_year <= '2015-2016' and sd.date_value <= to_date('01-08-2016','MM-DD-YYYY')
    ...
    LEFT OUTER JOIN
        ...
        FROM
            K12INTEL_DW.FTBL_DISCIPLINE d
            INNER JOIN K12INTEL_DW.DTBL_SCHOOL_DATES sd on d.school_dates_key = sd.school_dates_key and sd.local_school_year <= '2015-2016'
            LEFT OUTER JOIN K12INTEL_DW.FTBL_DISCIPLINE_ACTIONS da on d.discipline_key = da.discipline_key and da.discipline_action_type = 'Suspension'
        GROUP BY
            d.student_key,
            d.school_key)
    ...
    LEFT OUTER JOIN
        ...
        FROM
            ...
            INNER JOIN K12INTEL_DW.DTBL_SCHOOL_DATES sd on fm.school_dates_key = sd.school_dates_key  and sd.local_school_year <= '2015-2016' and sd.date_value <= to_date('01-08-2016','MM-DD-YYYY')
        WHERE
            fm.mark_type = 'Final'
        GROUP BY
            fm.student_key,
...
WHERE
   all_enr.enrollment_num = 1
    and (all_enr.first_enroll_year <=  '2015-2016' and all_enr.end_enroll_year >= '2015-2016' )
    and all_enr.previous_school_key is not null
    and listed_sch.school_code = '450'
GROUP BY
                st.student_id,
                st.student_key,
                listed_sch.school_code,

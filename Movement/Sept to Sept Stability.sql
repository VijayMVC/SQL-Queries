SELECT
    ...
    ,count(distinct first_date.student_key) as sept_first_year_students
    ,count(distinct sec_date.student_key) as sept_second_year_students
    ,round(count(distinct sec_date.student_key)/count(distinct first_date.student_key),3) as sept_sept_stability_rate
FROM
    (
    SELECT
        sa.student_key

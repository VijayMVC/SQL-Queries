SELECT
	...
	case when pre_post.post_natl_pct >= 76 then 1
                         ...
                         case when pre_post.post_natl_pct >= 76 then 'Significantly Above Target'
                                     ...
                                     when pre_post.post_natl_pct < 11 then 'Significantly Below Target' else 'Untested' end  Performance_Group
FROM
(SELECT
	st.student_id,
	sch.school_code,
	stu_list.test_subject,
	case when fall_map.rit is null then win_map.season else fall_map.season end as pre_season,
	case when fall_map.rit is null then win_map.test_student_grade else fall_map.test_student_grade end as pre_grade,
	case when fall_map.rit is null then win_map.rit else fall_map.rit end as pre_rit,

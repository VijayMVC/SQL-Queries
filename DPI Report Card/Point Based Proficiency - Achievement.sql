...
WHERE
	...
	AND sd.LOCAL_SCHOOL_YEAR in ('2010-2011','2011-2012', '2012-2013', '2013-2014')
...
SELECT
	...
	ach.school_name,
	ach.test_subject,
	ach.local_school_year,
	avg(ach.total_count) as avg_tested,
	case when fall_year = (select min(fall_year) from ach) then (1 * ach.total_count) as prior_year2_weight
	case when fall_year = (select min(fall_year) + 1 from ach) then (1.25 * ach.total_count) as prior_year1_weight
	case when fall_year = (select min(fall_year) + 2 from ach) then (1.5 * ach.total_count) as current_year_weight
FROM
(SELECT
	saa.SCHOOL_CODE,
	saa.school_name,
